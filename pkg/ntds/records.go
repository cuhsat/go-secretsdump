...
func (d *Reader) decryptSupp(record ese.Esent_record) (Supplemental, error) {
	...
	if len(bval) > 24 {                                   //is the value above the minimum for plaintex passwords?
		...
		var plainBytes []byte
		//check if the record is something? has a UPN?
		...
		for _, x := range props.Properties {
			...
			if strings.Compare(s, "Primary:Kerberos-Newer-Keys") == 0 {
				...
				r.KerberosKeys = make([]string, rec.CredentialCount)
				for credIndex := uint16(0); credIndex < rec.CredentialCount; credIndex++ {
					...
					if k, ok := kerbkeytype[keyData.KeyType]; ok {
						r.KerberosKeys[credIndex] = fmt.Sprintf("%s:%s:%s", username, k, hex.EncodeToString(keyVal))
					} else {
						r.KerberosKeys[credIndex] = fmt.Sprintf("%s:%d:%s", username, keyData.KeyType, hex.EncodeToString(keyVal))
					}
				}
			}
